...
def generate_cs(formula_string, lits, trace_length, num_pos, num_neg):
  """
  ...
  # failed to generate cs (can happen if formula reduces to true/false) or
  # CS exceeds requested number of traces
  if len(pos) == 0 or len(neg) == 0 or len(pos) > num_pos or len(neg) > num_neg:
      return None
  ...
  # repeat the characteristic set until we meet the requested number of examples
  repeat_pos = math.ceil(num_pos / len(new_pos))
  repeat_neg = math.ceil(num_neg / len(new_neg))
  new_pos = new_pos*repeat_pos
  new_neg = new_neg*repeat_neg
  new_pos = new_pos[:num_pos]
  new_neg = new_neg[:num_neg]
